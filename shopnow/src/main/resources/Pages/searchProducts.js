...
async function categRender() {
  document.getElementById("overlay").hidden = false;
  var products = await categFetch();
  ...
  if (products.Data.length == 0) {
    ...
    nd.style.width = "100%";
    nd.innerHTML = noproduct();
    productList.appendChild(nd);
  }
  ...
  setTimeout(() => (document.getElementById("overlay").hidden = true), 500);
}
...
function radioBTN(event) {
  if (
    event.attributes.value.nodeValue == "laptop" ||
    event.attributes.value.nodeValue == "mobile" ||
    event.attributes.value.nodeValue == "furniture" ||
    event.attributes.value.nodeValue == "gym" ||
    event.attributes.value.nodeValue == "shoes"
  ) {
    var radios = document.getElementsByTagName("input");
    for (let i = 0; i < radios.length; i++) {
      if (
        radios[i].attributes.name != undefined &&
        radios[i].attributes.name.nodeValue == "gender"
      ) {
  ...
  if (event.attributes.name.nodeValue == "gender")
    gender = event.attributes.value.nodeValue;
  else categ = event.attributes.value.nodeValue;
  categRender();
}
...
function noproduct() {
  ...
  <div class="empty-state__content">
    ...
    <div class="empty-state__message">No Product found.</div>
    <div class="empty-state__help">
      Try search for other products.
    </div>
  </div>
</div>`;
}
...
function clearfilter() {
  var radios = document.getElementsByTagName("input");
  for (let i = 0; i < radios.length; i++) {
    var curRadio = radios[i];
    if (
      curRadio.attributes.name != null &&
      (curRadio.attributes.name.nodeValue == "category" ||
        curRadio.attributes.name.nodeValue == "gender")
    ) {
